...
class EnglishExerciseView(QWidget):
    ...
    def init_ui(self):
        ...
        self.exercise_layout.addWidget(self.options_container_widget) # Add this container to the exercise layout
        ...
        self.exercise_layout.addWidget(self.answer_input)
    ...
    def _load_llm_question(self):
        ...
        try:
            topic = self.lesson_data.title if self.lesson_data else "General English" # Get topic from lesson_data
            logger.info(f"Loading 20 new English questions on topic '{topic}' (Difficulty: {self.current_difficulty_band})...")
